# Indomaret Pay One-Time Payment Integration

This example demonstrates how to integrate Indomaret Pay payments using PayPal's v6 Web SDK. Indomaret Pay is a popular payment method in Indonesia that allows customers to authorize payments through the Indomaret retail network.

## Architecture Overview

This sample demonstrates a complete Indomaret Pay integration flow:

1. Initialize PayPal Web SDK with the Indomaret Pay component
2. Check eligibility for Indomaret Pay payment method
3. Create Indomaret Pay payment session with required payment fields
...
6. Authorize the payment through Indomaret Pay popup flow
...
- Popup payment flow
- Eligibility checking for Indomaret Pay
...
3. **Enable Indomaret Pay**
   ...
   - Find **Indomaret Pay** in the payment methods and enable it
...
2. **Select Indomaret Pay Payments:**
   Click on the Indomaret Pay Payments link in the Static Examples section

## How It Works

### Geographic Availability

Indomaret Pay is available in Indonesia.

### Client-Side Flow

1. **SDK Initialization**: Loads PayPal Web SDK with the Indomaret Pay component using a client ID fetched from the server's `/paypal-api/auth/browser-safe-client-id` endpoint. `testBuyerCountry` is set to `"ID"` for sandbox testing.
2. **Eligibility Check**: Verifies Indomaret Pay is eligible for the merchant with IDR currency.
3. **Session Creation**: Creates an Indomaret Pay payment session (`createIndomaretPayOneTimePaymentSession`) with event callbacks for handling payment lifecycle events.
...
- `POST /paypal-api/checkout/orders/create-order-for-one-time-payment` - Create order with IDR currency and `ORDER_COMPLETE_ON_PAYMENT_APPROVAL` processing instruction
- `GET /paypal-api/checkout/orders/:orderId` - Fetch order details after approval

### Special Order Configuration

Indomaret Pay requires an order creation payload with:
...
### Indomaret Pay not eligible

- Verify `testBuyerCountry` is set to `"ID"`
- Check that `currencyCode` is set to `"IDR"` (Indonesian Rupiah)
- Ensure Indomaret Pay is enabled for the merchant in PayPal account settings
...
- **Ensure currency_code is set to `"IDR"`** — this is critical for Indomaret Pay

async function onPayPalWebSdkLoaded() {
  try {
    ...
    const sdkInstance = await window.paypal.createInstance({
      clientId,
      testBuyerCountry: "IT", // Italy for Bancomat Pay testing
      components: ["bancomatpay-payments"],
    });

    // Check if Bancomat Pay is eligible
    const paymentMethods = await sdkInstance.findEligibleMethods({
      currencyCode: "EUR",
    });

    const isBancomatPayEligible = paymentMethods.isEligible("bancomatpay");

    if (isBancomatPayEligible) {
      setupBancomatPayPayment(sdkInstance);
    } else {
      showMessage({
        text: "Bancomat Pay is not eligible. Please ensure your buyer country is Italy and currency is EUR.",
        type: "error",
      });
      console.error("Bancomat Pay is not eligible");
    }
  } catch (error) {
...
function setupBancomatPayPayment(sdkInstance) {
  try {
    // Create Bancomat Pay checkout session
    const bancomatPayCheckout = sdkInstance.createBancomatPayOneTimePaymentSession({
    ...
    setupPaymentFields(bancomatPayCheckout);

    // Setup button click handler
    setupButtonHandler(bancomatPayCheckout);
  } catch (error) {
    console.error("Error setting up Bancomat Pay payment:", error);
    showMessage({
...
function setupPaymentFields(bancomatPayCheckout) {
  // Create payment field for full name with optional prefill
  const fullNameField = bancomatPayCheckout.createPaymentFields({
    type: "name",
  ...
  document.querySelector("#bancomatpay-full-name").appendChild(fullNameField);
}

function setupButtonHandler(bancomatPayCheckout) {
  const bancomatPayButton = document.querySelector("#bancomatpay-button");
  bancomatPayButton.removeAttribute("hidden");
  document.querySelector("#phone-fields").removeAttribute("hidden");

  bancomatPayButton.addEventListener("click", async () => {
    try {
      const phoneData = validatePhoneFields();
      const isValid = await bancomatPayCheckout.validate();

      if (isValid) {
        await bancomatPayCheckout.start(
...
async function createOrderWithPhone(phoneData) {
  try {
    console.log("Creating PayPal order for Bancomat Pay...");
    const orderPayload = {
      intent: "CAPTURE",

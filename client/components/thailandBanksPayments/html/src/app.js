...
function setupPaymentFields(thailandBanksCheckout) {
  ...
  document.querySelector("#thailandbanks-full-name").appendChild(fullNameField);
}

function setupButtonHandler(thailandBanksCheckout) {
  const thailandBanksButton = document.querySelector("#thailandbanks-button");
  ...
  try {
    ...
    const response = await fetch(
      "/paypal-api/checkout/orders/create-order-for-one-time-payment",
      {
        ...
        body: JSON.stringify({ currencyCode: "THB" }),
      },
    );
